...
CRITICAL_NEUTRONS_PER_SECOND = 500
CRITICAL_PRODUCT_VALUE = 500_000
...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    temperature_condition = temperature < CRITICAL_TEMPERATURE
    neutrons_condition = neutrons_emitted > CRITICAL_NEUTRONS_PER_SECOND
    product_condition = product < CRITICAL_PRODUCT_VALUE

    return temperature_condition and neutrons_condition and product_condition
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    reactor_power = temperature * neutrons_produced_per_second

    safe_zone = threshold * 0.9
    nominal_zone = threshold * 1.1

    status = 'DANGER'

    if reactor_power < safe_zone:
        status = 'LOW'
    elif safe_zone <= reactor_power <= nominal_zone:
        status = 'NORMAL'
